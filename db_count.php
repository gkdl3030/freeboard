...
/* --- mysql서버와 데이터베이스 접속 --- */
$connet = mysqli_connect($host,$user,$passwd) or die("mysql서버 접속 에러");
mysqli_select_db($connect,'apm_db') or die("DB 접속 에러"); //apm_db 선택

$YY = date('Y');
$MM = date('m');
$DD = date('d');
$dat = $YY . "-" . $MM . "-" . $DD; //날짜 형식 지정 (예 : 2025-8-15)
...
/* ---조회 쿼리 실행 후 결과 셋에서 첫번째를 레코드를 할당 --- */
